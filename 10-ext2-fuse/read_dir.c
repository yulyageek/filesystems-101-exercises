...
int read_single_indirect_block(int img, __le32 block_nr, fuse_fill_dir_t filler, void *buf){
	if (block_nr == 0){
		return 0;
	}
	__u32 offset = block_nr * block_size;
	...
	for(__u32 i=0; i < (__u32)(block_size / sizeof(__le32)); i++){
		int ret = read_direct_block(img, single_inderect_block_buf[i], filler, buf);
		if(ret < 0){
			return ret;
		}
	}
...
int read_double_indirect_block(int img, __le32 block_nr, fuse_fill_dir_t filler, void *buf){
	if (block_nr == 0){
		return 0;
	}
	__u32 offset = block_nr * block_size;
	...
	for(__u32 i=0; i < (__u32)(block_size / sizeof(__le32)); i++){
		int ret = read_single_indirect_block(img, double_inderect_block_buf[i], filler, buf);
		if(ret < 0){
			return ret;
		}
	}
...
int dump_dir(int img, int inode_nr, fuse_fill_dir_t filler, void *buf)
{
	...
	for(int i = 0; i < 12; i++){
		ret = read_direct_block(img, in.i_block[i], filler, buf);
		if(ret < 0){
			return ret;
		}
	}
	...
	ret = read_single_indirect_block(img, in.i_block[12], filler, buf);
	if(ret < 0){
		return ret;
	}
	//14th block is single-indirect
	double_inderect_block_buf = (__le32*)malloc(block_size);
	ret = read_double_indirect_block(img, in.i_block[13], filler, buf);
	if(ret < 0){
		return ret;
	}
	return 0;
